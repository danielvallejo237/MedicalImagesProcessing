...
def CustomRotate(template,iteration=0,angulo=THETA,shape=(15,15))->array:
    plan=template.copy()
    degrees=iteration*angulo #Hay rotaciones de cada 15 grados
    (h, w) = plan.shape[:2]
    (cX, cY) = (shape[0] //2 , shape[1]//2)
    M=cv2.getRotationMatrix2D((cX, cY),degrees, 1.0)
    PlantillaRotada =cv2.warpAffine(plan.astype('float32'), M, (w, h))
    return PlantillaRotada

def CustomGMF(image,template,rotations=12)->array:
    imagenes=[]
    for i in range(rotations):
        Kernel=CustomRotate(template,i)
        resultante=ApplyConv(image,Kernel.astype('int'))
        imagenes.append(resultante)
    imagenes=np.asarray(imagenes)
    imagenes=np.max(imagenes,axis=0)
    return imagenes